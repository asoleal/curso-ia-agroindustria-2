...
\begin{infobox}{Objetivo}
Construir un script Bash (\texttt{scripts/setup\_proyecto.sh}) que automatice la creación de estructura, descarga de datos reales y validación básica.
\end{infobox}

\subsection{Conceptos Nuevos Necesarios}

Para este script usaremos herramientas de automatización que no hemos visto en detalle:

\begin{itemize}
    \item \textbf{\texttt{curl -o destino url}}: Descarga un archivo desde internet. Es como ``Guardar como...'' pero desde la terminal.
    \item \textbf{\texttt{if [ ! -f archivo ]; then ... fi}}: Bloque condicional que verifica si un archivo \textbf{NO} existe antes de intentar crearlo o descargarlo (evita sobreescribir trabajo previo).
    \item \textbf{\texttt{set -euo pipefail}}: Un ``modo estricto'' para Bash. Hace que el script se detenga inmediatamente si ocurre cualquier error, en lugar de seguir ejecutando comandos a ciegas.
\end{itemize}

\subsection{Requisitos Mínimos}

\begin{enumerate}
    \item \textbf{Automatización de Carpetas:} El script debe crear \texttt{data/raw}, \texttt{data/processed}, \texttt{scripts} y \texttt{reports} usando \texttt{mkdir -p} (para no fallar si ya existen).
    \item \textbf{Descarga Automática:} Debe descargar el dataset Iris desde una URL pública (ver plantilla) y guardarlo como \texttt{data/raw/datos\_sensores.csv}.
    \item \textbf{Validación de Calidad:} Generar un archivo \texttt{reports/validacion\_inicial.txt} con:
    \begin{itemize}
        \item Fecha de ejecución (\texttt{date}).
        \item Conteo de filas y columnas.
        \item Detección de posibles valores vacíos (\texttt{grep ",,"}).
    \end{itemize}
    \item \textbf{Control de Versiones:} El repositorio final debe tener un \texttt{.gitignore} adecuado y al menos 3 commits descriptivos.
\end{enumerate}

\subsection{Plantilla del Script (Base)}

Copia este código en \texttt{scripts/setup\_proyecto.sh} y estúdialo. Ya incluye la URL correcta para el ejercicio.
...
\begin{warningbox}{Tips de Ejecución}
Recuerda dar permisos de ejecución antes de probarlo:
\texttt{chmod +x scripts/setup\_proyecto.sh}\\
Luego ejecútalo desde la raíz del proyecto:
\texttt{./scripts/setup\_proyecto.sh}
\end{warningbox}

\subsection{Entrega}

\begin{itemize}
    \item Sube tu repositorio a GitHub.
    \item Asegúrate de que el \texttt{README.md} explique cómo ejecutar tu script de setup.
    \item Comparte la URL del repositorio.
\end{itemize}
